...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-api.git --from 0.3.0
```

Then add `GoogleApi` to your target's dependencies:

```bash
swift package add-target-dependency GoogleApi <target-name> --package swift-google-api
```
